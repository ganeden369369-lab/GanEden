...
- `pnpm --filter @gan-eden/mobile exec expo start` — then open in Expo Go on your device/simulator.
...
`pnpm --filter @gan-eden/mobile exec expo start --web` runs the app in a browser. It's a convenience for reviewing UI without a simulator, not a supported release target (see `docs/ARCHITECTURE.md` §3.1, D7). The design-system component gallery is at the `/dev/gallery` route.
